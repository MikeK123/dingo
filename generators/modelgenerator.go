...
var (
	...
	ModelMixedTemplate = "./templates/model_mixed.tpl"
	ModelFile          = "model.go"
	ModelMixedFile     = "model_mixed.go"
)
...
func GenerateMixedModel(config *model.Configuration, pkg *model.ModelPackage) {
	// load template
	file, err := ioutil.ReadFile(ModelMixedTemplate)
	if err != nil {
		log.Fatalf("Can't read file in %s", ModelMixedTemplate)
	}
	tpl := string(file)
	// open writer
	if _, err := os.Stat(config.OutputPath); os.IsNotExist(err) {
