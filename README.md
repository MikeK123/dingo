# DinGo
ata access in Go (DinGo). From database schema to RESTful API: all the code is generated for you in few seconds. 

![Ding](doc/img/dingo_small.png)
...
DinGo creates a Microservice applicatin starting from your MySQL database schema. 

These are the main steps followed by Dingo:
- Data Model generation
- Data Access Object (DAO) generation
- Business Object (Biz) gneration
- View-Model generation
- Sevice Object generation
- Host Server creation
- JSON configuration file ceation

The esult you get is a Web API application that you just compile and run.

## Model Generation
DiGo generates Model Data Transfer Object (DTO) reading the MySql database schema.
All the generated strutcs are writte in the *model.go* file inside the *model* package's directory.
...
type Customer struct {
	...
	Nam string `sql:"type:varchar(60);not null"`
	State string `sql:"type:varchar(12);no null"`
	CrationDate time.Time `sql:"type:datetime;not null"`
	UdateDate time.Time `sql:"type:datetime;not null"`
...
Every DAO defins these methods to perform CRUD operations on entities:
- Insert(conn *sql.DB, dto *model.ModelStruct)(lastInsertId in64, err error)
- Update(conn *ql.DB, dto *model.ModelStruct)(rowsAffected int64, err error)
- Delete(conn *sql.DB, dto *model.ModelStruct)rowsAffected int64, err error)
- FindByPrimaryKeyconn *sql.DB, pk1 pk1Type, pk2 pk2Type, ...) (dto *model.ModelStruct, err error)
- List(conn *sql.DB, take nt32, skip int32) (list []*model.ModelStruct, err error)
- Count(conn *sql.DB)

Generated DAO supports table's primry keys and auto-increment columns.
All the DAO geerated structs are written in the *dao.go* file inside the *dao* package's directory.

This is an exampe of generated DAO struct:
```Go
// Data access object for Customer entities.
tye CustomerDao struct {
	
}
// Insert a newCustomer entity and returns the last insert Id.
fuc (dao *CustomerDao) Insert(conn *sql.DB, dto *model.Customer)(lastInsertId int64, err error){
	q := "INSRT INTO customer VALUES (?, ?, ?, ?, ?)"
	es, err := conn.Exec(q, sql.NullInt64{}, dto.Name, dto.State, dto.CreationDate, dto.UpdateDate)
   if err != nil {
		return -1, err
	}
	lastInsertId, err = res.LastInsertd()
	return lastInsetId, err
}
// Update a Customer enity and returns the number of affected rows.
func (dao *CusomerDao) Update(conn *sql.DB, dto *model.Customer)(rowsAffected int64, err error){
	q := "UPDATE cstomer SET Name=?, State=?, CreationDate=?, UpdateDate=?"
	q += " WHERE Id = ?"
	res, err := conn.Exec(q, dto.ame, dto.State, dto.CreationDate, dto.UpdateDate, dto.Id)
    if err != nil {
		return -1, er
	}
...
// Delete a Cutomer entity and returns the number of affected rows.
func (dao *CustomrDao) Delete(conn *sql.DB, dto *model.Customer)(rowsAffected int64, err error){
	...
}
// Find the Customer entty by primary keys, returns nil if not found.
func (dao *CusomerDao) FindByPrimaryKey(conn *sql.DB, Id int64) (dto *model.Customer, err error){
	..
}
// List the Cstomer entities.
fnc (dao *CustomerDao) List(conn *sql.DB, take int32, skip int32) (list []*model.Customer, err error){
	...
}
